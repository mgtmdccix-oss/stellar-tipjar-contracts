...
a
    ...
    pub fn rf_create_round(
        ...
        Self::require_not_paused(&env);
        admin.require_auth();
        let whitelisted: bool = env.storage().instance().get(&DataKey::TokenWhitelist(token.clone())).unwrap_or(false);
        if !whitelisted { panic_with_error!(&env, TipJarError::TokenNotWhitelisted); }
        retroactive_funding::create_round(&env, &admin, &token, reward_pool, criteria, voting_start, voting_end)
    }

    /// Nominates a project with impact metrics. Checked against round's EvalCriteria.
    ...
    pub fn rf_nominate_project(
        ...
        Self::require_not_paused(&env);
        retroactive_funding::nominate_project(&env, round_id, &project, tip_volume, unique_tippers, impact_description);
    }
    ...
    /// Casts token-weighted votes for a project. One vote per voter per round.
    /// Emits `("rf_cast",)` with data `(round_id, voter, project, weight)`.
    pub fn rf_cast_vote(env: Env, voter: Address, round_id: u64, project: Address, weight: i128) {
        Self::require_not_paused(&env);
        voter.require_auth();
    ...
    pub fn rf_get_project(env: Env, round_id: u64, project: Address) -> Option<retroactive_funding::ProjectRecord> {
